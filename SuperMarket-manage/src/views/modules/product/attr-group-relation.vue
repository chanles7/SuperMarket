...
export default {
  ...
  methods: {
    ...
    batchDeleteRelation(val) {
      ...
      this.$http({
        url: this.$http.adornUrl("/product/attrgrouprelation/delete"),
        method: "post",
    ...
    submitAddRealtion() {
      ...
      if (this.innerdataListSelections.length > 0) {
        ...
        this.$http({
          url: this.$http.adornUrl("/product/attrgrouprelation/save"),
          method: "post",
          data: this.$http.adornData(postData, false),
        }).then(({ data }) => {
          if (data.code == 200) {
    ...
    getDataList() {
      ...
      this.$http({
        url: this.$http.adornUrl(
          "/product/attrgroup/" + this.attrGroupId + "/attr/norelation"
        ),
        method: "get",
        params: this.$http.adornParams({
          page: this.pageIndex,
